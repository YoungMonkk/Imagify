...
export const generateImage = async (req, res) => {
    try {
        ...
        if (!apiKey || apiKey === 'replace_with_clipdrop_key') {
            console.error('ClipDrop API key is missing or not configured');
            return res.status(500).json({
                success: false,
                message: "ClipDrop API key is not configured. Please check your server configuration."
            });
        }

        console.log('Making ClipDrop API request with key:', apiKey.substring(0, 20) + '...');
